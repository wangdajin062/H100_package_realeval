...
| 实验 | 名称 | 核心数据 | 状态 |
|---|---|---|---|
| exp1 | QAD Production Distillation（QAD 生产蒸馏） | `{"experiment": "exp1", "computation": "h100_real_qwen", "f1": 0.4256, "accuracy": 0.66, "n_train": 12800, "n_test": 3200, "kl_final": 0.36553, "drift_pct_final": 61.479, "kl_plateau": 0.809274, "kl_converged": 0.456622, "total_steps": 2000, "ovf_activation_step": 1400, "snr_min": 3.4, "snr_max": 4.6, "quantize": "int4", "is_synthetic": false}` | ✅ |
...
| exp11 | Quantization Scheme（量化方案） | `{"experiment": "exp11", "computation": "h100_real_qwen", "schemes": {"fp16": {"f1": 0.3125, "accuracy": 0.4541}, "int8": {"f1": 0.3452, "accuracy": 0.5353}, "int4": {"f1": 0.4287, "accuracy": 0.6644}, "nf4": {"f1": 0.3072, "accuracy": 0.4644}}, "model_source": "exp1_qad"}` | ✅ |
...
| exp14 | Multi-model same-data comparison（多模型同数据对比） | `{"experiment": "exp14", "computation": "h100_real_qwen", "models": {"bf16_0.5b_transformers": {"f1": 0.5853, "runtime": "transformers", "source": "ours"}, "q4km_0.5b_llama_cpp": {"f1": 0.7025, "latency_ms_p50": 14.02, "runtime": "llama_cpp", "source": "ours"}}` | ✅ |
...
- **f1**：`0.4256`（2026-08-01 重跑更新，exp1_qad 曾被 exp10 覆盖后已恢复）
- **accuracy**：`0.66`
- **n_train**：`12800`
- **n_test**：`3200`
- **kl_final**：`0.36553`
...
- **experiment**：`exp11`
- **computation**：`h100_real_qwen`
- **schemes**：`{"fp16": {"f1": 0.3125, "accuracy": 0.4541}, "int8": {"f1": 0.3452, "accuracy": 0.5353}, "int4": {"f1": 0.4287, "accuracy": 0.6644}, "nf4": {"f1": 0.3072, "accuracy": 0.4644}}`（2026-08-01 修复 quantize 传递 + exp1_qad 覆盖后重跑）
- **model_source**：`exp1_qad`
...
- **experiment**：`exp14`
- **computation**：`h100_real_qwen`
- **models**：`{"bf16_0.5b_transformers": {"f1": 0.5853, "runtime": "transformers", "source": "ours"}, "q4km_0.5b_llama_cpp": {"f1": 0.7025, "latency_ms_p50": 14.02, "runtime": "llama_cpp", "source": "ours"}}`（2026-08-01 重编译 CUDA llama-cpp 后 GPU 推理成功）
