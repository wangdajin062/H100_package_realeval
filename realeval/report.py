...
def build_paper_figures(fmt: str = "png") -> list:
    ...
    if comp and fp:
        pts = []
        if "QAD_MultiGuard_INT4" in comp:
            pts.append(("QAD+OVF INT4", comp["QAD_MultiGuard_INT4"]["f1"], fp.get("0.5B_Q4_K_M", 240)))
        if "FraudFusion_pruned_INT4" in comp:
            pts.append(("FraudFusion INT4", comp["FraudFusion_pruned_INT4"]["f1"], fp.get("0.5B_Q4_K_M", 240) * 1.1))
        # SAFE-QAQ 7B is a cite-only baseline; only plot if exp12 provides the value from its results.
        safeqaq = comp.get("SAFE_QAQ_7B_cited")
        if safeqaq and safeqaq.get("f1") is not None:
            pts.append(("SAFE-QAQ 7B BF16 (cited)", safeqaq["f1"], fp.get("7B_BF16_SAFE_QAQ")))
        fig, ax = plt.subplots(figsize=(7, 4))
