...
def load_spam11358(max_samples: int | None = None) -> dict:
...
def load_chifraud_balanced(max_samples: int | None = None) -> dict:
    ...
    if path.exists():
        texts, labels = _load_jsonl(path, max_samples)
...
def prepare_sft_dataset(name_or_path: str = None, max_samples: int | None = None,
    ...
    logger.info("SFT data (manual split): %d train (%.1f%% fraud) / %d test (%.1f%% fraud)",
                len(train_texts), 100 * sum(train_labels) / max(1, len(train_labels)),
                len(test_texts), 100 * sum(test_labels) / max(1, len(test_labels)))
    return (train_texts, train_labels), (test_texts, test_labels)
...
def load_text_corpus(name: str, max_samples: int | None = None) -> dict:
    """Load a text corpus by short name.

    Supported names:
        ``advfraud`` / ``advfraud3k``  → ``load_advfraud3k``
        ``balanced4k``                  → ``load_chifraud_balanced``
        ``chifraud``                    → ``load_chifraud``
        ``taf28k``                      → ``load_taf28k``
        ``taf28k_npz``                  → ``load_taf28k`` with source='npz'
        ``synthetic``                   → ``load_synthetic``
        ``spam11358``                   → ``load_spam11358``
    ...
    loader_map = {
        "advfraud": load_advfraud3k,
        "advfraud3k": load_advfraud3k,
        "balanced4k": load_chifraud_balanced,
        "chifraud": load_chifraud,
        "taf28k": lambda ms=None: load_taf28k(max_samples=ms, source="auto"),
        "taf28k_npz": lambda ms=None: load_taf28k(max_samples=ms, source="npz"),
        "synthetic": lambda ms=None: load_synthetic(n=(ms or 100)),
        "spam11358": load_spam11358,
    }
...
def load_dataset(name: str = "balanced4k", max_samples: int | None = None, **kwargs) -> dict:
    """Backward-compatible wrapper around load_text_corpus.

    Formerly a registry-based loader; now delegates to ``load_text_corpus``.
    Kept for compatibility with experiments that call ``data.load_dataset(...)``.
    """
    return load_text_corpus(name, max_samples=max_samples)
...
def group_split(
    ...
    Args:
        texts: List of text samples (used for length, not content).
        labels: Corresponding labels — can be int, str, or any hashable type.
        test_ratio: Fraction of samples to hold out for testing.
        seed: Random seed for reproducibility.

    Returns:
        (train_indices, test_indices) where each is a list of int indices
        into the original ``texts`` / ``labels`` arrays.
    """
    n = len(texts)
    assert len(labels) == n, f"texts ({n}) and labels ({len(labels)}) length mismatch"
    if n == 0:
        return [], []
    ...
    # Final shuffle so training order isn't label-clustered
