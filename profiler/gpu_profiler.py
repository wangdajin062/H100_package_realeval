"""profiler/gpu_profiler.py — Independent GPU Profiler

Separate from benchmark logic. Currently profiles:
  - GPU: memory, utilization, power, SM clock, temperature, PCIe

(CPU/disk/network profiling are planned but not yet implemented.)
...
class GpuProfiler:
    ...
    def save_csv(self) -> Path | None:
        """Save raw samples to CSV. Returns None if no samples collected."""
        if not self._samples:
            return None
