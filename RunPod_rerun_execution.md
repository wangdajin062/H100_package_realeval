...
> 依据：`rerun_checklist.md`（P0/P1/P2 分级）× 2026-08-02 字段对齐审核（D 盘 `0a722b1` 已推送 GitHub）
...
# 3) 代码同步到最新（0a722b1，含 check_alignment.py + 字段对齐修复）
cd /workspace/H100_package_realeval
git fetch origin && git checkout main && git reset --hard origin/main
git log --oneline -1        # 应为 0a722b1
...
# 绘图字段对齐（4 处 std MISSING 的告警——若走路径 A 则保持【已追踪】状态，不视为失败）
...
2. 更新 `RUNLOG_2026MMDD.md`，提交：
   ```bash
   cd /d/Projects/H100_package_realeval
   git add outputs/results RUNLOG_*.md && git commit -m "results: 同步 RunPod 重跑结果" && git push origin main
   ```
3. 回填论文 `v25_blind.tex` 前，逐项对照 `rerun_checklist.md` 判定（P0 结论反转项先改结论再改数字）。
