...
def run(config: dict) -> dict:
    ...
    def run_paper(config):
        ...
        for dname, ds in datasets.items():
            if ds["texts"]:
                ...
                # AdvFraud-3k is all-fraud â balance with normal samples from taf28k
        ...
        if "taf28k" in results:
            ...
            out["balanced4k"] = results["taf28k"]  # alias for paper_data bridge
    ...
    def run_smoke(_: dict) -> dict:
        ...
        for eps in (0.5, 1.0, 1.5, 3.0):
            ...
            ldp[f"eps_{eps}"] = {"epsilon": eps, "f1": classification_metrics(yte, clf_dp.predict(Xte))["f1"]}
