...
class OVFreeze:
    ...
    def _make_hook(self, name):
        def hook(_module, _inp, out):
            ...
            self.reg_terms.append(((var_est - sigma2) ** 2).sum())    # Eq. (eq:ovf-loss)

            # Eq. (eq:ovf-rescale-en) as written has no bound on c_l. In finite-precision
            # training a covered layer's batch variance can transiently collapse toward
            # zero (especially once the task loss has already converged, as it has by
            # step ~65 on this saturated setup), which sends sigma2/var -> inf and the
            # rescale explodes the layer's output. Until the paper's Appendix
            # (app:bounded, "boundedness of the correction factor") is confirmed to
            # specify a bound, clamp c to a conservative range as a documented,
            # non-paper safety measure.
            c = torch.sqrt(sigma2 / (var_batch + self.eps)).detach()
            c = c.clamp(min=0.1, max=10.0)
    ...
    def loss(self):
        if not self.reg_terms:
            return None
        total = torch.stack(self.reg_terms).sum() * self.lam
        self.reg_terms = []
        return total
