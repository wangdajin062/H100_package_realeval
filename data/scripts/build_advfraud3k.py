...
构建方法（论文 §实验设置与数据基准）:
  ...
  Step 2: 对抗式改写（8 种策略）
  Step 3: 复核 (Cohen's κ — NOT MEASURED, requires human annotation review)
...
def build_advfraud3k() -> list[dict]:
    ...
    # Step 3: Agreement verification
    # Cohen's kappa requires human annotation review. This script generates
    # the dataset; kappa must be computed separately with real annotator judgments.
    print("[Step 3] 复核 — agreement value NOT MEASURED (requires human annotation)")
    # In real usage, human annotators would review. Here we mark all as approved
    # for inclusion but do NOT assign a fabricated agreement score.
    for s in samples:
        s["reviewed"] = True
        s["agreement"] = "NOT_MEASURED"  # Cohen's kappa: requires human annotation review
...
def build_args():
    import argparse
    ap = argparse.ArgumentParser(description="Build AdvFraud-3k adversarial dataset")
    ap.add_argument("--seed", type=int, default=42, help="Random seed (default: 42)")
    ap.add_argument("--n-fraud", type=int, default=1000, help="Fraud samples from TAF-28k (default: 1000)")
    ap.add_argument("--n-novel", type=int, default=2000, help="Novel fraud templates (default: 2000)")
    ap.add_argument("--taf-path", type=Path, default=None,
                    help="Path to taf28k.jsonl (default: data/TAF28k/taf28k.jsonl)")
    ap.add_argument("--out-dir", type=Path, default=None,
                    help="Output directory (default: data/AdvFraud3k)")
    return ap.parse_args()


def main():
    args = build_args()
    global SEED, N_FRAUD_SAMPLES, N_NOVEL_FRAUD, TOTAL, TAF_TEST_PATH, OUTPUT_DIR, OUTPUT_JSON, OUTPUT_JSONL
    SEED = args.seed
    N_FRAUD_SAMPLES = args.n_fraud
    N_NOVEL_FRAUD = args.n_novel
    TOTAL = N_FRAUD_SAMPLES + N_NOVEL_FRAUD
    TAF_TEST_PATH = args.taf_path or TAF_TEST_PATH
    OUTPUT_DIR = args.out_dir or OUTPUT_DIR
    OUTPUT_JSON = OUTPUT_DIR / "advfraud3k.json"
    OUTPUT_JSONL = OUTPUT_DIR / "advfraud3k.jsonl"
    random.seed(SEED)
    ...
    print(f"  seed={SEED}  n_fraud={N_FRAUD_SAMPLES}  n_novel={N_NOVEL_FRAUD}")
    print("=" * 60)
    print()
